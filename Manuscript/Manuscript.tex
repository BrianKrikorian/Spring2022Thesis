...
The coefficients for each position are listed in
Table~\ref{tab:Coefficientstable}. Coefficients are subjective, but were changed
...
\begin{equation}
 \label{eq:PAR}
  ...
  {\text{Points Per Win}}.
\end{equation}
Equation~\eqref{eq:PAR} gives a good idea of how many points a player accounts for 
during a 
...
\jy{Refer to a table or figure like this:}
...
\jy{The two tables for guards could be combined into one; and the same for
  forwards}
...
\jy{don't scale figures; control their sizes in R when generating them;}
\begin{figure}[tbp]
  \centering
...
\jy{two many digits in this table}
